#!/usr/bin/env node
// Renderiza "蒼キ嬰児" a MP4 fotograma a fotograma con Chromium headless.
//
...
async function openPage(url) {
  ...
  await page.goto(url);
  const info = await page.evaluate(() => window.AE.ready);
...
const size = opt('size', '1920x1080').split('x');
const url = `http://127.0.0.1:${srv.address().port}/index.html?render=1&w=${size[0]}&h=${size[1]}&s=${opt('scale', '0.6667')}`;
const t0 = Date.now();

try {
  if (opt('shots')) {
    // --shots "sea@2.5,touch@0.6" → plano@instante_local
    const outdir = path.resolve(opt('outdir', 'stills'));
    fs.mkdirSync(outdir, { recursive: true });
    const { browser, page } = await openPage(url);
    for (const spec of opt('shots').split(',')) {
      const [name, lt] = spec.split('@');
      const t1 = Date.now();
      const data = await page.evaluate(([a, b]) => window.AE.shot(a, b), [name, +lt]);
      const f = path.join(outdir, `shot_${name}_${lt}.png`);
    ...
    for (const s of opt('stills').split(',').map(Number)) {
      const t1 = Date.now();
      const data = await page.evaluate(t => window.AE.still(t), s);
    ...
    const { browser, page } = await openPage(url);
    const b64 = await page.evaluate(() => window.AE.audio());
    ...
    const out = path.resolve(opt('out', 'aoki-eiji.mp4'));
    ...
    await Promise.all(pages.map(async ({ page }) => {
      // de dos en dos: los planos animados "a doses" reutilizan el render 3D del fotograma par
      while (next < to) {
        const i = next; next += 2;
        for (const k of [i, i + 1]) {
          if (k >= to) break;
          const b64 = await page.evaluate(n => window.AE.frame(n), k);
          ready.set(k, Buffer.from(b64, 'base64'));
        }
    ...
    if (!noAudio) {
      ...
      const b64 = await pages[0].page.evaluate(() => window.AE.audio());
